...
variable "product" {
  description = "Product this datastore belongs to. Pinned to \"streaming-hub\" by validation: the derived name (streaming-hub-{env}-valkey) and the derived secret path (streaming-hub-{env}-valkey/auth-token) ARE the cross-stack discovery contract. A different value here silently produces resources the streaming-hub Helm release cannot find. A second product gets its own directory under examples/aws/products, not a different value here."
  type        = string
  default     = "streaming-hub"

  validation {
    condition     = var.product == "streaming-hub"
    error_message = "The product must be \"streaming-hub\". This root stack is the streaming-hub Valkey datastore; every name and secret path it produces is derived from it. To provision Valkey for another product, copy this directory to examples/aws/products/<product>/valkey instead."
  }
}
...
# from module.naming here would produce streaming-hub-{env}-vpc /
# streaming-hub-{env}-eks, which do not exist — which is why this root stack does
...
# reason: the streaming-hub workloads run on the EKS nodes, and the replication group has to be
...
variable "eks_cluster_name" {
  description = "Name of the EKS cluster whose node security group is resolved. Leave empty (the default) to DERIVE \"lerian-{environment}-eks\" — the cluster belongs to infra-base and carries the \"lerian\" product label, NOT \"streaming-hub\". The lookup matches the node security group by tag:Name = \"{cluster}-node\", which is what terraform-aws-modules/eks sets."
  type        = string
  default     = ""
}
...
variable "port" {
  description = "Port the cache nodes accept connections on. The hub has NO separate port variable — STREAMING_HUB_REDIS_ADDRESS carries \"host:port\" in one string, as the chart's own example shows (charts/streaming-hub/values-template.yaml:39)."
  type        = number
  default     = 6379
}
