...
# the Valkey datastore of the streaming-hub product
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/streaming-hub/valkey/terraform.tfstate). Its siblings —
# postgres, msk and secrets — are independent roots with independent state, so a
# change to one can never queue behind an apply of another and a corrupt state
...
# module seeded with product = "streaming-hub" would produce streaming-hub-{env}-vpc and
# streaming-hub-{env}-eks, which do not exist.
################################################################################
...
################################################################################
# Valkey — streaming-hub-{environment}-valkey
# Secret: streaming-hub-{env}-valkey/auth-token   Host: the raw primary endpoint
################################################################################
